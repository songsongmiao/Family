...
public class NestedRefresh extends ViewGroup implements NestedScrollingParent{
    ...
    private RefreshControl mSwipeControl;                                             //刷新头部控制器
    ...
    private RefreshControl.SwipeModel mModel = RefreshControl.SwipeModel.SWIPE_BOTH;    //刷新模式设置
    private int scrollY_Up;
    private int scrollY_Down;
    private OnRefreshListener mOnRefreshListener;


    public NestedRefresh(Context context) {
        this(context, null);
    }

    public NestedRefresh(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NestedRefresh(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        mParentHelper = new NestedScrollingParentHelper(this);
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Refresh);
            int modelIndex = a.getInt(R.styleable.Refresh_fresh_model, -1);
            if (modelIndex >= 0 && modelIndex < RefreshControl.SwipeModel.values().length) {
                mModel = RefreshControl.SwipeModel.values()[modelIndex];
            }
            a.recycle();
        }
        initSwipeControl();
    ...
    private void initSwipeControl() {
        mSwipeControl = new RefreshControlStyleNormal(getContext());
        View headerView = mSwipeControl.getSwipeHead();
        View footView = mSwipeControl.getSwipeFoot();
        addView(headerView, 0);
        addView(footView, getChildCount());
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mModel == RefreshControl.SwipeModel.SWIPE_NONE || mModel == RefreshControl.SwipeModel.SWIPE_ONLY_LOADINN) {
            scrollY_Up = 0;
        }

        if (mModel == RefreshControl.SwipeModel.SWIPE_NONE || mModel == RefreshControl.SwipeModel.SWIPE_ONLY_REFRESH) {
            scrollY_Down = 0;
        }
    ...
    private int offSetChildrenLocation(int deltaOriginY, boolean pre) {
        ...
        if (currentScrollY != 0 || !pre) {
            ...
            if (swipeViewVisibilityHei > 0) {                                                                                          //更新刷新状态
                if (mIsFreshContinue) {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_LOADING, swipeViewVisibilityHei, mSwipeControl.getSwipeHead().getHeight());
                } else if (mIsFreshComplete) {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_COMPLETE, swipeViewVisibilityHei, mSwipeControl.getSwipeHead().getHeight());
                } else if (willTo < middleHei) {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_OVER, swipeViewVisibilityHei, mSwipeControl.getSwipeHead().getHeight());
                } else {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_TOAST, swipeViewVisibilityHei, mSwipeControl.getSwipeHead().getHeight());
                }
            }
            ...
            if (willTo > 0) {
                if (mIsLoadComplete) {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_FOOT_COMPLETE, willTo, mSwipeControl.getSwipeFoot().getHeight());
                } else {
                    mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_FOOT_LOADING, willTo, mSwipeControl.getSwipeFoot().getHeight());
                }
            }
        ...
        if (isOverProgress) {
            ...
            animationReBackToRefreshing.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animation) {
                    if (!isCancel) {
                        mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_LOADING, mSwipeControl.getSwipeHead().getHeight(), mSwipeControl.getSwipeHead().getHeight());
                        if (!mIsFreshContinue) {
    ...
    public void finishRefresh() {
        postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_HEAD_COMPLETE,
    ...
    public void setIsLoadComplete(boolean isLoadComplete) {
        ...
        mSwipeControl.onSwipeStatue(RefreshControl.SwipeStatus.SWIPE_FOOT_COMPLETE,
...
//        }
    ...
    // 设置刷新模式
    public void setSwipeModel(RefreshControl.SwipeModel model) {
    ...
    // 设置自定义刷新视图
    public void setSwipeControl(RefreshControl control) {
        if (control != null && this.mSwipeControl != control) {
            ...
            addView(control.getSwipeHead());
            addView(control.getSwipeFoot());
            requestLayout();
        }
    }
